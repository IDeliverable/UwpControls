...
namespace IDeliverable.Controls.Uwp.ScrollSelector
{
    ...
    public sealed class ScrollSelector : ListView
    {
        ...
        public ScrollSelector()
        {
            ...
            mPointerUiHideTimer = new Timer(PointerUiHideTimerTick, null, Timeout.InfiniteTimeSpan, Timeout.InfiniteTimeSpan);
        }

        private readonly Timer mScrollDelayTimer;
        private readonly Timer mPointerUiHideTimer;
        ...
        protected override void OnApplyTemplate()
        {
            ...
            AddHandler(PointerMovedEvent, new PointerEventHandler((sender, e) => ShowPointerUi(e)), handledEventsToo: true);
            AddHandler(PointerPressedEvent, new PointerEventHandler((sender, e) => ShowPointerUi(e)), handledEventsToo: true);
        ...
        private async void ScrollDelayTimerTick(object state)
        {
            await Dispatcher.RunAsync(CoreDispatcherPriority.Normal, SelectScrolledItem);
        }

        private async void PointerUiHideTimerTick(object state)
        {
            await Dispatcher.RunAsync(CoreDispatcherPriority.Normal, () => VisualStateManager.GoToState(this, mPointerUiHiddenStateName, useTransitions: true));
        }

        private void ShowPointerUi(PointerRoutedEventArgs e)
        {
            ...
            if (e.Pointer.PointerDeviceType == PointerDeviceType.Mouse || e.Pointer.PointerDeviceType == PointerDeviceType.Pen)
            {
                ...
                mPointerUiHideTimer.Change(TimeSpan.FromSeconds(2.5), Timeout.InfiniteTimeSpan);
            }
        }
